...
- **非管理员会话后台生图完成图无法发送**：框架 `send_message_to_user` 工具限制非管理员会话只能发送工作区/临时目录内的本地文件，插件数据目录下的生成图会被拦截。聊天来源（指令/LLM 工具/批量）的生成图改为直接保存到会话工作区下的 `astrbot_plugin_gemini_image_generation/` 子目录，AI 可直接发送；工作区解析失败时本次生成直接报错，不再静默落盘到无法投递的位置。容量上限 `image_cache_max_size_mb` 对聊天会话按全部工作区插件名子目录总量生效（防跨会话累积膨胀），其他来源维持插件数据目录按目录清理；旧版框架（无会话工作区）及工作台/插件 API 来源维持插件数据目录存储。

## [3.0.6] - 2026-09-09

### Added

- **Responses 生图渠道**：新增 `openai_responses`，顶层模型默认 `gpt-5.6-luna`、生图模型默认 `gpt-image-2.5-flare`，均支持自由输入。支持文生图、参考图编辑、完整 JSON 和 SSE 响应，以及质量、透明背景、输出格式、压缩率、生成/编辑模式和自动尺寸配置。
- **生成中预览**：Responses 和 OpenAI Images 可请求部分图片，在工作台运行中的任务卡展示临时缩略图；预览不进入画廊、不写入历史文件、不计入最终图片数量。连续编辑沿用上一轮图片作为参考图。

### Changed

- **OpenAI Images 适配 GPT Image 2.5**：支持 Flare/Sunburst 及日期快照，新增配置默认使用 Flare；已有模型配置不自动改写。GPT Image 2.5 支持 `xhigh/max` 质量，旧模型保留对应参数范围。
- **尺寸、批量与原生流式返回**：GPT Image 2/2.5 预设尺寸按分辨率和比例推导，支持自动尺寸及符合官方约束的自定义尺寸；非流式 GPT Image 请求支持配置单次 1–10 张，流式每次一张，多图继续由插件调度。
- **压缩率与 API 语义一致**：JPEG/WebP 的 `output_compression` 按 0–100 原值发送，默认改为 `100`，PNG 不发送此参数。**旧配置若使用 `0` 表示服务端默认值，请改成 `100`。** 所接网关仍需支持对应协议和模型，Images 路由能力可能与 Responses 工具不同。

### Fixed

- **快速模式附图调用**：头像、海报、壁纸、卡片、手机、手办化不再强制要求额外描述，修复只附图时提示“必要参数缺失”的问题；可选风格编号和附加描述继续有效，表情包原有可选描述行为保持不变。
- **GPT Image 编辑参数**：编辑接口同步传递质量、背景、格式、压缩率和审核参数；多图使用 `image[]`，最多 16 张，保留 PNG/JPEG/WebP 对应上传类型。GPT Image 生成和编辑均不再发送不支持的 `response_format`，保留旧 DALL·E 兼容路径。
- **生成结果与重试边界**：只交付完整图片，正确处理原生图片 SSE 与 base64 响应；已提交后超时、断流或结果未知时停止自动重试和跨候选切换，避免重复生成。GPT Image 不发送非官方 seed 参数，透明背景与 JPEG 的不兼容组合在发送前提示。

## [3.0.5] - 2026-09-09

### Added

- **外部插件生图接口（API v1）**：同一 AstrBot 实例内的其他插件可通过 `get_service(api_version=1)` 查询就绪状态与候选能力、提交多图生成任务、查询或等待结果、取消任务及注册完成回调；无需聊天事件，不自动发送图片或唤醒聊天 Agent。详见 [其他插件接入指南](docs/plugin-api.md)。
- **统一生成调度**：指令、LLM、Studio 与外部插件共用 FIFO 队列，默认同时执行 3 个生成调用、最多等待 100 个；新增 `generation_max_concurrency` 和 `generation_max_queue_size` 配置，重载生效。排队不占供应商请求超时，队列满时拒绝提交，原有批量与供应商局部并发限制继续生效。
- **外部插件任务来源与限流**：Studio 任务、画廊及详情展示调用插件名称、标识和回调状态，支持来源及插件筛选。调用插件默认自行限流，也可显式接入本插件规则：有 UMO 时共用会话额度，无 UMO 时按插件标识独立使用默认额度。
- **SenseAudio 供应商**：新增 `senseaudio` 模板，接入同步图片生成与异步提交/查询协议，支持 Image 2.0、Image 1.0、Doubao Seedream 5.0 Lite 和 SenseNova U1 Fast、单张参考图、随机种子及模型尺寸白名单；Studio 能力与参考图上限由后端提供。异步查询受剩余总预算约束，轮询失败不在当前候选内重新提交任务。图生图省略尺寸时由服务端适配，不保证保持参考图原始像素尺寸。

### Fixed

- **任务持久化失败时保留图片结果**：外部插件任务收尾时，任务或历史记录写入失败不再遮蔽已生成图片；当前进程内仍可查询终态、释放等待者并尝试完成回调，回调状态与生成状态独立。磁盘写入失败的结果不保证重启后恢复。
- **限制任务默认等待时间**：`wait_task()` 未指定超时时使用插件 `total_timeout`；等待超时或取消等待不会取消后台生成，调用方仍可再次查询或等待。
- **本地帮助页字体下载**：优先使用 `astrdark.cyou` 镜像，失败后回退 Noto 官方 GitHub；支持插件代理与代理环境变量，代理失败后尝试直连。下载内容经字体解析验证后再保存，避免将 HTTP 200 错误页当成字体；字体校验和写入移出事件循环。
...
- **全部供应商按官方文档完成逐模型参数能力查证与落地**：Google 系（google/vertex/gemini_interactions）新增按模型家族的能力画像——lite 仅 1K（工作台不再显示 2K/4K，运行时降级）、lite 支持极端比例与 thinking（修正旧实现误伤）、2.5 系不注入 image_size、lite/2.5 不发送搜索接地；xai 比例白名单补 21:9/5:2、编辑参考图上限 3→5、quality 增 auto、分辨率收窄 1K/2K；minimax 按模型分层（21:9 与 2K 仅 image-01，剔除不存在的 4:5/5:4）；dashscope 4K 档仅 wan2.7-image-pro 可选；openai_images 的 style 仅 dall-e-3 生效、gpt-image 系不再发送 response_format；stepfun 模板标注编辑模型 2026-10-10 下线。
- **vertex 错误报告修复**：安全过滤代码只从 `raiFilteredReason` 字段识别（此前全量正则扫描会把 404 报错里的项目号误报为安全拦截）；404/NOT_FOUND 归类为「模型或端点不存在」且不可重试。
...
- **LLM 工具比例枚举并集化**：`aspect_ratio` 全局枚举（工具 schema、校验）改为各渠道官方比例并集（新增 Gemini 极端比例与 xAI 宽幅枚举），LLM 调用可请求 `1:4`/`1:8`/`4:1`/`8:1`/`2:1`/`1:2`/`19.5:9`/`9:19.5`/`20:9`/`9:20`；xAI 能力档案改按官方白名单精确声明（含 `auto`，不含 `4:5`/`5:4`/`21:9`）。工具文案不再罗列取值，改为引导先调用 `gemini_image_provider_models` 查询。
...
- 供应商配置表每条条目新增 `enabled` 开关（默认 `true`），可临时停用单条模型配置而不删除它；禁用条目不参与轮询和生图，LLM 模型查询工具也检索不到，同渠道其他条目不受影响。
...
- 生图命令和 LLM 工具支持按供应商、原始模型或模型别名路由；模型别名可跨供应商复用，最终统计展示实际成功的供应商、原始模型和别名。
- 新增 `gemini_image_provider_models` 能力查询工具，默认返回供应商、模型、别名和文生图/改图模式，详细查询仅返回生图工具实际支持的参数。
- 新增持久化后台任务号与 `gemini_image_task_status` 查询工具；记录按会话隔离并默认保留 24 小时，插件重启后未完成任务标记为中断。
- `gemini_image_generation` 新增命名 `batch_tasks`，支持配置任务数、单任务图片数和并发上限，并按供应商原生批量上限拆分、补齐和汇总发送。
- LLM 工具新增负面提示词、水印、质量和保留参考图尺寸参数，并按候选能力在请求前过滤。

### Changed

- 未显式指定供应商或模型时保持原有轮询；显式指定后不再越过匹配范围降级到其他候选。
- 前台等待或发送阶段超时时返回包含任务号和路由状态的 JSON，后台完成后继续向用户发送结果。
...
- [docs/新增API供应商.md](docs/新增API供应商.md)：补全 sensenova / stepfun / provider_limits / reference_intake / data_uri 文件树并新增“公共辅助模块”章节
- [tl/README.md](tl/README.md)：registry 表补全 sensenova；新增“Provider 公共辅助模块”章节与 sensenova / stepfun / format_error / api_headers 接口表；`tl_utils.format_error_message` 标记为 re-export shim
- [docs/config.md](docs/config.md)：补全 sensenova provider、新增 `sensenova_settings` 章节与 11 种官方固定尺寸映射表

### CI / Build

- 新增 `.github/workflows/code-quality.yml`（Ruff lint + format-check + `compileall` + metadata/json schema 校验）
- `.gitignore` 新增 `plans/`，不再跟踪本地实施计划草稿
