...
- **非管理员会话后台生图完成图无法发送**：框架 `send_message_to_user` 工具限制非管理员会话只能发送工作区/临时目录内的本地文件，插件数据目录下的生成图会被拦截。聊天来源（指令/LLM 工具/批量）的生成图改为直接保存到会话工作区 `generated_images/` 子目录，AI 可直接发送；工作区解析失败时本次生成直接报错，不再静默落盘到无法投递的位置。容量上限 `image_cache_max_size_mb` 对聊天会话按全部工作区 `generated_images/` 总量生效（防跨会话累积膨胀），其他来源维持插件数据目录按目录清理；旧版框架（无会话工作区）及工作台/插件 API 来源维持插件数据目录存储。
