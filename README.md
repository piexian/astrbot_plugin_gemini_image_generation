...
- **内置 WebUI 创作台**：Dashboard 插件页「studio」——在线工作台（模型扁平直选、单图多张与批量生成、参考图上传/画廊拾取）、全来源生成任务实时进度（SSE）、历史画廊（筛选/灯箱/下载/删除/再次生成/用作参考图）；生成历史持久化归档，存量图片自动迁入。详见 [使用指南](docs/usage.md#webui-创作台)。
